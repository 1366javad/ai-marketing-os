...
This document is the concrete companion to `context-slicing-matrix.md`. The matrix lists *which* fields and canonical `module + artifact` identities each module reads — this document traces *actual* `getCampaignContextSlice()` calls with realistic input/output, so every ambiguous edge case is resolved here, in writing, before `getCampaignContextSlice.js` is implemented.
...
No edge case here — Research has no artifact dependencies (per the Matrix), so `relevantEvents` is empty on a fresh campaign and stays empty until something else has run.
...
**Resolved edge case:** Content reads an approved Research artifact
(`research + audience_analysis` in this example) AND an approved SEO artifact
(`seo + keyword_cluster`) — both, not either/or. This was implicit in the Matrix
but is now explicit: a Content call without keyword context would produce
SEO-blind drafts, which defeats the point of campaign-mode in the first place.
...
**Resolved edge case (your first question):** Creative reads `content + blog_draft`/`content + email_draft` *only when `approvalStatus: "approved"`* — `evt_007` appears here because it was approved; an equivalent pending draft would not appear. This was stated in the Matrix ("only if approved") but is now shown concretely: the filtering isn't a separate step, it's baked into what `relevantEvents` contains by the time it reaches the agent.

**Second resolved point:** when Creative itself later produces *both* `creative + creative_concept` and `creative + image_asset` from the same call, both are written as separate Memory Events (not bundled into one event with two artifacts) — so a future caller reading Creative's output sees two distinct events, each independently approvable. A carousel concept can be approved while its accompanying image asset is sent back for revision, without blocking each other.
...
**Resolved edge case (your second question):** Ads does **not** receive
`evt_007` (`content + blog_draft`) in `relevantEvents`, even though it was
approved and visible to Creative one step earlier. This is intentional, not a
gap: per the Matrix, Ads reads `seo + seo_strategy` and approved
`creative + creative_concept`/`creative + image_asset`; `content + blog_draft`
is not in Ads' read list. Ad copy is written from positioning + approved
creative direction, not from long-form blog content.
...
```js
{
  ...
  relevantEvents: [
    // every approved/auto_saved event in the campaign, regardless of module + artifact —
...
1. **Creative → `creative + creative_concept` and `creative + image_asset`:** both are read when present and approved; both are written as separate, independently-approvable events.
2. **Ads → `content + blog_draft`:** explicitly excluded. Ads reads approved SEO strategy and Creative artifacts, not raw blog content.
